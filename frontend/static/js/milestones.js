...
async function loadMilestones() {
    ...
    // hint: GET /orgs/{orgId}/teams/{teamId}/projects/{projId}/milestones
    const milestonesRes = await ___
...
function renderMilestones(milestones) {
    ...
    list.innerHTML = milestones.map(m => `
        <div class="milestone_item" data-milestone-id="${m.id}">
            ...
            <div class="milestone_body">
                <div class="milestone_title">${___}</div>
                <div class="milestone_desc">${___ ?? ''}</div>
    ...
    list.querySelectorAll('[data-delete-id]').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            // TODO: DELETE the milestone, then reload
            ___
        })
    })
}
...
document.getElementById('cancel_milestone_btn').addEventListener('click', () => {
...
newMilestoneForm.addEventListener('submit', async (e) => {
    ...
    // TODO: POST new milestone, close modal, reload
    ___
})
