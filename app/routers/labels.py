...
async def get_task_labels(org_id: UUID, team_id: UUID, proj_id: UUID, label_id: UUID, current_user = Depends(validate_user), db: AsyncSession = Depends(db_session)):
    return await label_service.get_task_labels(org_id, team_id, proj_id, label_id, current_user.id, db)


@label_router.post("/{label_id}/task_label", response_model=TaskLabelResponse)
async def create_task_label(org_id: UUID, team_id: UUID, proj_id: UUID, label_id: UUID, data: TaskLabelCreate, current_user = Depends(validate_user), db: AsyncSession = Depends(db_session)):
    return await label_service.create_task_label(org_id, team_id, proj_id, label_id, data, current_user.id, db)


@label_router.get("/{label_id}/task_label/{task_label_id}", response_model=TaskLabelResponse)
async def get_task_label(org_id: UUID, team_id: UUID, proj_id: UUID, label_id: UUID, task_label_id: UUID, current_user = Depends(validate_user), db: AsyncSession = Depends(db_session)):
    return await label_service.get_task_label(org_id, team_id, proj_id, label_id, task_label_id, current_user.id, db)


@label_router.delete("/{label_id}/task_label/{task_label_id}")
async def delete_task_label(org_id: UUID, team_id: UUID, proj_id: UUID, label_id: UUID, task_label_id: UUID, current_user = Depends(validate_user), db: AsyncSession = Depends(db_session)):
    return await label_service.delete_task_label(org_id, team_id, proj_id, label_id, task_label_id, current_user.id, db)
