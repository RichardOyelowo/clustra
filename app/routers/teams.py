...
async def get_team(org_id: uuid.UUID, db: AsyncSession = Depends(db_session)):
    pass


@team_router.patch("/{team_id}")
async def edit_team(org_id: uuid.UUID, db: AsyncSession = Depends(db_session)):
    pass


@team_router.delete("/team_id}")
async def delete_team(org_id: uuid.UUID, db: AsyncSession = Depends(db_session)):
    pass
